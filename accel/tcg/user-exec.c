...
static int dump_region(void *priv, vaddr start,
                       vaddr end, unsigned long prot)
{
    FILE *f = (FILE *)priv;
...
{
    const int length = sizeof(vaddr) * 2;
 ...
 */
void page_set_flags(vaddr start, vaddr end, int flags)
{
    vaddr last;
    bool reset = false;
    bool inval_tb = false;
...
int probe_access_flags(CPUArchState *env, vaddr addr,
...
void page_reset_target_data(vaddr start, vaddr end)
{
