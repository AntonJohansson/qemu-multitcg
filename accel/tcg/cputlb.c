...
int probe_access_full(CPUArchState *env, vaddr addr, int size,
                      ...
                      uintptr_t retaddr)
{
    int flags = probe_access_internal(env_cpu(env), addr, size, access_type, mmu_idx,
                       ...
                       bool nonfault, void **phost, uintptr_t retaddr)
{
    ...
    flags = probe_access_internal(env_cpu(env), addr, size, access_type, mmu_idx,
 ...
 */
static uint64_t do_ld_mmio_beN(CPUState *cpu, MMULookupPageData *p,
                               ...
                               MMUAccessType type, uintptr_t ra)
{
    ...
    QEMU_IOTHREAD_LOCK_GUARD();
    for (i = 0; i < size; i++) {
        uint8_t x = io_readx(cpu, full, mmu_idx, addr + i, ra, type, MO_UB);
        ret_be = (ret_be << 8) | x;
    }
    return ret_be;
}
                          ...
                          MemOp mop, uintptr_t ra)
{
    ...
    if (unlikely(p->flags & TLB_MMIO)) {
        return do_ld_mmio_beN(cpu, p, ret_be, mmu_idx, type, ra);
    }
                          ...
                          uint64_t a, int mmu_idx, MemOp mop, uintptr_t ra)
{
    ...
    if (unlikely(p->flags & TLB_MMIO)) {
        p->size = size - 8;
        a = do_ld_mmio_beN(cpu, p, a, mmu_idx, MMU_DATA_LOAD, ra);
        p->addr += p->size;
        p->size = 8;
        b = do_ld_mmio_beN(cpu, p, 0, mmu_idx, MMU_DATA_LOAD, ra);
                        ...
                        MMUAccessType type, MemOp memop, uintptr_t ra)
{
    ...
    if (unlikely(p->flags & TLB_MMIO)) {
        return io_readx(cpu, p->full, mmu_idx, p->addr, ra, type, memop);
    }
                          ...
                          MemOpIdx oi, uintptr_t ra)
{
    ...
    if (likely(!crosspage)) {
        ...
        if (unlikely(l.page[0].flags & TLB_MMIO)) {
            QEMU_IOTHREAD_LOCK_GUARD();
            a = io_readx(cpu, l.page[0].full, l.mmu_idx, addr,
                         ra, MMU_DATA_LOAD, MO_64);
            b = io_readx(cpu, l.page[0].full, l.mmu_idx, addr + 8,
                         ra, MMU_DATA_LOAD, MO_64);
...
static void plugin_load_cb(CPUState *cpu, abi_ptr addr, MemOpIdx oi)
{
    qemu_plugin_vcpu_mem_cb(cpu, addr, oi, QEMU_PLUGIN_MEM_R);
}
...
uint8_t cpu_ldb_mmu(CPUArchState *env, abi_ptr addr, MemOpIdx oi, uintptr_t ra)
{
    ...
    plugin_load_cb(env_cpu(env), addr, oi);
    return ret;
}
 ...
 */
static uint64_t do_st_mmio_leN(CPUState *cpu, MMULookupPageData *p,
                               uint64_t val_le, int mmu_idx, uintptr_t ra)
{
    ...
    for (i = 0; i < size; i++, val_le >>= 8) {
        io_writex(cpu, full, mmu_idx, val_le, addr + i, ra, MO_UB);
    }
    return val_le;
}
                          ...
                          MemOp mop, uintptr_t ra)
{
    ...
    if (unlikely(p->flags & TLB_MMIO)) {
        return do_st_mmio_leN(cpu, p, val_le, mmu_idx, ra);
                            ...
                            MemOp mop, uintptr_t ra)
{
    ...
    if (unlikely(p->flags & TLB_MMIO)) {
        p->size = 8;
        do_st_mmio_leN(cpu, p, int128_getlo(val_le), mmu_idx, ra);
        p->size = size - 8;
        p->addr += 8;
        return do_st_mmio_leN(cpu, p, int128_gethi(val_le), mmu_idx, ra);
    } else if (unlikely(p->flags & TLB_DISCARD_WRITE)) {
...
{
    if (unlikely(p->flags & TLB_MMIO)) {
        io_writex(cpu, p->full, mmu_idx, val, p->addr, ra, memop);
    } else if (unlikely(p->flags & TLB_DISCARD_WRITE)) {
        /* nothing */
    } else {
                        ...
                        MemOpIdx oi, uintptr_t ra)
{
    ...
    if (likely(!crosspage)) {
        ...
        if (unlikely(l.page[0].flags & TLB_MMIO)) {
            ...
            io_writex(cpu, l.page[0].full, l.mmu_idx, a, addr, ra, MO_64);
            io_writex(cpu, l.page[0].full, l.mmu_idx, b, addr + 8, ra, MO_64);
